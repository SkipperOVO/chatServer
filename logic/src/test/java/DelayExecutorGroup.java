...
public class DelayExecutorGroup {
    ...
    public DelayExecutorGroup(int nthread, int taskQueueCap)  {
        this.nthread = nthread;
        workers = new DelayedExecutor[nthread];
        for (int i = 0; i < nthread; ++i) {
            workers[i] = new DelayedExecutor(taskQueueCap);
        }
    }
    ...
    public void execute(Runnable task, Integer bindId) {
        ...
        for (int i = 0; i < nthread; ++i) {
            if (workers[i].isBind(bindId)) {
                workers[i].scheduleWithFixedDelay(task);
                hasBind = true;
            }
        }
        if (hasBind == false) {
            ...
            workers[ri].scheduleWithFixedDelay(task);
        }
    }
